...
\begin{enumerate}
    ...
    \item Créer une infrastructure de développement professionnel,
    \item Mettre en place un pipeline DevOps complet en passant par les étapes suivantes:
          \begin{enumerate}
              \def\labelenumii{\alph{enumii}.}
              \item Mettre en place un outil d'intégration continue (CI),
              \item Mettre en place un outil de déploiement continue (CD),
              \item Mettre en place une infrastructure de production accueillant le rendu du projet.
          \end{enumerate}
    \item Mettre à jour le projet existant vers les nouvelles versions des technologies utilisées
    \item Reconcevoir la base de données pour qu'elle prenne en compte:
          \begin{enumerate}
              \def\labelenumii{\alph{enumii}.}
              \item Les rôles des utilisateurs,
              \item Les ressources liées aux machines industrielles utilisées dans l'application.
          \end{enumerate}
    \item
          Améliorer le backend du projet en passant par les étapes suivantes:
          \begin{enumerate}
              \def\labelenumii{\alph{enumii}.}
              \item Appliquer les standards de programmation web (REST),
              \item Mettre en place une architecture de code cohérente,
              \item Mettre en place une vérification des entrées utilisateurs,
              \item Mettre en place l'authentification de l'utilisateur,
              \item L'ajout des rôles des utilisateurs,
              \item Ajouter la gestion des ressources liées aux machines industrielles utilisées dans l'application,
              \item Améliorer le système de notifications pour le rendre plus sécurisé.
          \end{enumerate}
    \item
          Mettre à jour le frontend pour qu'il intègre les changements du backend en passant par les étapes suivantes:
          \begin{enumerate}
              \def\labelenumii{\alph{enumii}.}
              \item Mettre à jour le frontend pour coller aux modifications du backend,
              \item Ajouter la gestion des ressources liées aux machines industrielles utilisées dans l'application,
              \item Améliorer le système de notifications pour le rendre plus sécurisé.
          \end{enumerate}
    \item Tester l'application à l'aide de tests automatisés.
\end{enumerate}
...
Avant de commencer, définissons l'infrastructure de développement.
...
\begin{itemize}
    ...
    \item un SGBD (Système de Gestion de Base de Données), https://fr.wikipedia.org/wiki/Syst%C3%A8me_de_gestion_de_base_de_donn%C3%A9es
...
\begin{itemize}
    \item le SGBD (Système de Gestion de Base de Données), https://fr.wikipedia.org/wiki/Syst%C3%A8me_de_gestion_de_base_de_donn%C3%A9es
...
En résumé, tous les outils ont été installé sur la machine de développement sans virtualisation.
%\figi{infrastructure_dev_actuelle.xml}{10cm}{Infrastructure de développement actuelle}
%\fig{infrastructure_dev_actuelle.xml}{Infrastructure de développement actuelle}

%\figi{infrastructure_dev_db_docker.xml}{10cm}{Infrastructure de développement avec une db end docker}
%\fig{infrastructure_dev_db_docker.xml}{Infrastructure de développement avec une db end docker}

%\figi{infrastructure_dev_db_laravel_docker.xml}{10cm}{Infrastructure de développement avec une db et laravel en docker}
...
Inconvénients:
\begin{itemize}
    \item Prends beaucoup de temps à installer,
    \item Coûte beaucoup en mémoire sur le pc,
...
\begin{itemize}
    ...
    \item Coûte peu en mémoire sur le pc,
...
\begin{itemize}
    ...
    \item Configuration en gande partie déjà réalisée,
    \item Compatible avec tous les OS,
    \item Existance d'un outil conçu pour être utiliser avec Docker (sail)
...
%\figi{infrastructure_prod_actuelle.xml}{10cm}{Infrastructure de production actuelle}
%\fig{infrastructure_prod_actuelle.xml}{Infrastructure de production actuelle}

%\figi{infrastructure_prod_db_docker.xml}{10cm}{Infrastructure de production avec une db en docker}
%\fig{infrastructure_prod_db_docker.xml}{Infrastructure de production avec une db en docker}

%\figi{infrastructure_prod_db_laravel_docker.xml}{10cm}{Infrastructure de production avec une db et laravel en docker}
...
\subsection{DevOps}
%\figi{devops_dockerhub.xml}{10cm}{DevOps via dockerhub}
%\fig{devops_dockerhub.xml}{DevOps via dockerhub}

%\figi{devops_heroku.xml}{10cm}{DevOps via heroku}
%\fig{devops_heroku.xml}{DevOps via heroku}
https://devcenter.heroku.com/articles/getting-started-with-laravel

%\figi{devops_github.xml}{10cm}{DevOps via github}
%\fig{devops_github.xml}{DevOps via github}

% code
\subsection{Architectures de code}
%\figi{architecture_code_spring.xml}{10cm}{Architecture de code du framework Spring "classique"}
%\fig{architecture_code_spring.xml}{Architecture de code du framework Spring "classique"}

%\figi{architecture_code_laravel.xml}{10cm}{Architecture de code du framework Laravel "classique"}
...
\chapter{Conception du Backend / de l'API}

\chapter{Intégration continu (CI)}

\chapter{Déploiement continu (CD)}
